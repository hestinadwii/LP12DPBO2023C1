...
public class Controller extends KeyAdapter implements KeyListener
{
    ...
    public synchronized void keyPressed(KeyEvent e)
    {
        ...
        if(game.isRunning())
        {
            ...
            /** PENGECEKAN ARAH **/
            // prev key disimpen ke temp dulu
            // kalo temp direction (key arah yang sebelumnya) beda ama key yang sekarang
            // maka set nilai si temp dengan key arah yang sekarang
            if((key == KeyEvent.VK_W) || (key == KeyEvent.VK_UP))
            {
                // Move up.
                temp.setVelY(-5);
                // pengecekan arah
                if (game.getTempDirection() != key)
                {
                    game.setTempDirection(key);
                    game.setScore(+1); // game.setScore diset +1 semua agar pertambahan skor tiap objek bergerak adalah 1
                }
            }
